...
module CocinaMetadata
  ...
  class DescriptionComponent < ViewComponent::Base
    FORMS_TO_EXCLUDE = ['genre', 'reformatting quality', 'map scale', 'map projection'].freeze
    ...
    # TODO: should this include publication event notes such as issuance, see: kj040zn0537
    def imprint_hash
    ...
    def form_type_label(hash)
      hash['displayLabel'] || (['resource type', 'extent'].include?(hash['type']) ? hash['type'] : 'form')
    end
    ...
    # TODO:  Should this instead be form.extent.notes?
    # See https://purl.stanford.edu/cf386wt1778.json
    def extent_notes
      @extent_notes ||= @document.cocina_record.path('$.description.form[?(@.type == "extent")].note.*').to_a
    end
