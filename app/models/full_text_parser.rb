...
class FullTextParser
  ...
  delegate :cocina_doc, :bare_druid, to: :purl_object
  ...
  def full_text_url(file_name)
    "#{::Settings.stacks.file_url}/#{bare_druid}/#{ERB::Util.url_encode(file_name)}"
  end
  ...
  def text_transcription_files
    resource_contains(cocina_doc).flat_map do |resource|
      resource_contains(resource).select do |file|
        file.fetch('filename', '') == "#{bare_druid}.txt"
      end
    end
  end
  ...
  def transcription_resources
    resource_contains(cocina_doc).select do |resource|
